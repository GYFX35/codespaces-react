    ...
    try:
        with open(csv_filepath, mode="r", encoding="utf-8") as csvfile:
            ...
            if not reader.fieldnames:
                print(
                    f"Error: CSV file at {csv_filepath} appears to be empty or has no header row."
                )
                return None

            # Check for required headers
            if (
                "SourceDevice" not in reader.fieldnames
                or "TargetDevice" not in reader.fieldnames
            ):
                print(
                    f"Error: CSV file must contain 'SourceDevice' and 'TargetDevice' columns. Found: {reader.fieldnames}"
                )
                return None
            ...
            for row in reader:
                ...
                if not source or not target:
                    print(
                        f"Warning: Skipping row with empty source or target device: {row}"
                    )
                    continue
        ...
        for node in nodes:
            if not graph.has_node(
                node
            ):  # Though add_edge should handle this if source/target were valid
                graph.add_node(node)

        if not graph.nodes():
            print(
                "Warning: No nodes found in the graph. The CSV might be empty or all rows were invalid."
            )
...
def draw_network_graph(graph, output_filepath, layout_type="spring"):
    ...
    if layout_type == "spring":
        pos = nx.spring_layout(
            graph, k=0.15, iterations=20
        )  # k adjusts spacing, iterations for convergence
        ...
        print(
            f"Warning: Unknown layout type '{layout_type}'. Defaulting to spring layout."
        )
    ...
    try:
        ...
        plt.savefig(
            output_filepath, format="PNG", dpi=150
        )  # Save as PNG with decent DPI
...
def export_graph_gexf(graph, output_filepath):
    ...
    if (
        not graph
    ):  # Check if graph is None or empty (no nodes might still be valid for an empty file)
...
def export_graph_graphml(graph, output_filepath):
    ...
    try:
        nx.write_graphml(
            graph, output_filepath, infer_numeric_types=True
        )  # infer_numeric_types can be useful
        return True
    except Exception as e:
...
"""
    ...
    if network_graph:
        print(
            f"Graph created successfully with {network_graph.number_of_nodes()} nodes and {network_graph.number_of_edges()} edges."
        )
        ...
        success_png = draw_network_graph(
            network_graph, output_png_path, layout_type="spring"
        )
        if success_png:
            print(
                f"PNG generated: {output_png_path} (Please check this file visually if running locally)"
            )
        ...
        success_png_kk = draw_network_graph(
            network_graph, output_png_path_kk, layout_type="kamada_kawai"
        )
        if success_png_kk:
        ...
        if non_existent_graph is None:
            ...
            )  # Export functions will also be skipped as graph is None
        ...
        if network_graph:  # Only attempt if graph building was successful
            ...
            if success_gexf:
                print(
                    f"GEXF exported: {output_gexf_path} (Verify with Gephi or similar tool if running locally)"
                )
        ...
        if network_graph:  # Only attempt if graph building was successful
            ...
            if success_graphml:
                print(
                    f"GraphML exported: {output_graphml_path} (Verify with Gephi or similar tool if running locally)"
                )
        ...
        if (
            empty_graph and not empty_graph.nodes()
        ):  # Should create a graph, but it will be empty
            print(
                f"Handled empty CSV. Nodes: {empty_graph.number_of_nodes()}, Edges: {empty_graph.number_of_edges()}"
            )
