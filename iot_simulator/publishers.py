...
def format_payload(device_id, timestamp, sensor_data_list):
    ...
    """
    payload = {"deviceId": device_id, "timestamp": timestamp}
...
def send_http_post(url, payload, timeout=10):
    ...
    try:
        ...
        print(
            f"HTTP error sending data to {url}: {e.response.status_code} {e.response.reason}"
        )
...
if __name__ == "__main__":
    ...
    test_sensor_data_list = [
        test_sensor_data_temp,
        test_sensor_data_humidity,
        test_sensor_data_gps,
    ]

    # --- Test format_payload ---
    print("--- Testing Payload Formatting ---")
    formatted_payload = format_payload(
        test_device_id, test_timestamp, test_sensor_data_list
    )
    ...
    print(
        "This test will likely print success if the endpoint is reachable and returns 2xx."
    )
    ...
    if success_http:
        print(
            "HTTP POST test reported success (check Beeceptor or your test endpoint)."
        )
    else:
        print(
            "HTTP POST test reported failure (this is expected if no internet/server)."
        )

    print("\n--- Testing HTTP POST with invalid URL ---")
    success_http_invalid = send_http_post(
        "http://nonexistentfakedomain123abc.com/api", http_payload
    )
