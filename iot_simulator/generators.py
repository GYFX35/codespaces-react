...
def generate_temperature(
    min_temp=DEFAULT_TEMP_RANGE_CELSIUS[0], max_temp=DEFAULT_TEMP_RANGE_CELSIUS[1]
):
...
def generate_humidity(
    min_humidity=DEFAULT_HUMIDITY_RANGE_PERCENT[0],
    max_humidity=DEFAULT_HUMIDITY_RANGE_PERCENT[1],
):
...
DEVICE_STATES = (
    {}
)  # Key: device_id, Value: dict of sensor states (e.g., last_lat, last_lon, last_count)
...
def generate_sensor_data(device_id, sensor_type_config):
    ...
    if sensor_type == "temperature":
        ...
        DEVICE_STATES[device_id]["counter"] = generate_counter_value(
            DEVICE_STATES[device_id]["counter"]
        )
        return {"event_count": DEVICE_STATES[device_id]["counter"]}
    else:
        print(
            f"Warning: Unknown sensor type '{sensor_type}' requested for device '{device_id}'."
        )
        return None
...
if __name__ == "__main__":
    ...
    gps_coords2 = generate_gps_coordinates(
        gps_coords1["latitude"], gps_coords1["longitude"]
    )
    ...
    sensor_reading = generate_sensor_data(
        dev1, "gps"
    )  # Second GPS reading for same device
    print(f"Sensor data (GPS again): {sensor_reading}")
    assert (
        sensor_reading["location"]["latitude"] != BASE_LATITUDE
        or sensor_reading["location"]["longitude"] != BASE_LONGITUDE
    )
    ...
    sensor_reading = generate_sensor_data(
        dev1, "counter"
    )  # dev1 counter should be unaffected
