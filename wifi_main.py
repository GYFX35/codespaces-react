...
def main():
    ...
    if not target_interface:
        ...
        if not target_interface:
            print(
                "Could not auto-detect Wi-Fi interface. Please specify one using --interface."
            )
            return
        ...
        if args.scan:
            if networks_data:
                ...
                for net in networks_data:
                    ...
                    print(
                        f"    Channel: {net['channel']} (Band: {net['band']}, Freq: {net['frequency']})"
                    )
    ...
    if args.channels:
        if (
            networks_data is None
        ):  # Should have been populated if --scan wasn't also specified
            print(
