...
def get_channel_usage(networks_data):
    ...
    return dict(sorted(channel_usage_2_4ghz.items())), dict(
        sorted(channel_usage_5ghz.items())
    )
...
def display_channel_usage_text(channel_usage_2_4ghz, channel_usage_5ghz):
    ...
    if channel_usage_2_4ghz:
        ...
        for channel in range(1, 15):  # Common 2.4 GHz channels (1-14)
            ...
            if (
                count > 0
            ):  # Only print channels in use or if we want to show all, then remove this if
                bar = ""
                if max_count_2_4 > 0:  # Avoid division by zero
                    bar_len = (
                        int((count / max_count_2_4) * max_bar_len)
                        if max_count_2_4 > 0
                        else 0
                    )
                    bar = "█" * bar_len
                ...
                channel_usage_2_4ghz and channel in channel_usage_2_4ghz
            ):  # Explicitly show if 0 but was in data
    ...
    print(
        "Aim for channels 1, 6, or 11 where possible in crowded 2.4 GHz environments."
    )
