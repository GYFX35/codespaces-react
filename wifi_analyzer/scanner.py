...
def get_wifi_interface():
    ...
    try:
        ...
        for line in result.stdout.splitlines():
            if line.strip().startswith('Interface '):
                ...
                if interface.startswith(('wlan', 'wlp', 'wlx', 'ath')): # Common Wi-Fi interface prefixes
                    return interface
    ...
    try:
        ...
        result = subprocess.run(['nmcli', '-t', '-f', 'DEVICE,TYPE', 'dev', 'status'], capture_output=True, text=True, check=True)
        for line in result.stdout.splitlines():
            parts = line.strip().split(':')
            if len(parts) == 2 and parts[1] == 'wifi':
                return parts[0] # Device name
    except (FileNotFoundError, subprocess.CalledProcessError):
        print("Error: Could not automatically determine Wi-Fi interface using 'iw dev' or 'nmcli'.")
        print("Please ensure 'iw' or 'nmcli' (NetworkManager) is installed and in your PATH.")
    return None
        ...
        if len(parts) >= 5: # SSID, CHAN, FREQ, SIGNAL, SECURITY[, EXTRA_STUFF_IF_ANY]
            ...
            try:
                ...
                networks.append({
                    ...
                    "band": band,
                    "frequency": freq_str,
                    "security": security if security else "Open" # Assume open if security is empty
                })
            except ValueError:
...
def scan_wifi_networks(interface=None):
    ...
    try:
        subprocess.run(['nmcli', 'dev', 'wifi', 'rescan', 'ifname', interface],
                       capture_output=True, text=True, timeout=10) # Allow some time for rescan
    except FileNotFoundError:
        print("Error: nmcli command not found. Please ensure NetworkManager is installed.")
        return []
    except subprocess.TimeoutExpired:
        print("Warning: nmcli rescan command timed out. Proceeding with potentially stale data.")
    except subprocess.CalledProcessError as e:
        print(f"Warning: nmcli rescan failed: {e.stderr}")
        # Continue anyway, list might still work with cached data

    cmd = ['nmcli', '-t', '-f', 'SSID,CHAN,FREQ,SIGNAL,SECURITY', 'dev', 'wifi', 'list', 'ifname', interface]
...
""" # nmcli -t -f ... does not print header if there is data
    ...
    for net in parsed_networks_mock:
        print(f"  SSID: {net['ssid']}, Signal: {net['signal']}, Chan: {net['channel']}, Band: {net['band']}, Security: {net['security']}")

    assert len(parsed_networks_mock) == 4 # "EmptySignal" fails int conversion for signal
    ...
    parsed_no_networks = parse_nmcli_output("SSID:CHAN:FREQ:SIGNAL:SECURITY") # Only header
