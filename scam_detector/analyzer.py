...
SUSPICIOUS_URL_PATH_KEYWORDS = ["login", "verify", "account", "secure", "update", "signin", "banking", "password"]
...
def analyze_text_for_scams(text_content):
    ...
    for url_str in found_urls:
        ...
        domain_to_check = parsed_url.hostname if parsed_url else url_str # Fallback to full string if parse failed
        ...
        for keyword in SUSPICIOUS_URL_PATH_KEYWORDS:
            if keyword in url_str.lower(): # Check the whole URL string
                ...
                if parsed_url and parsed_url.hostname:
                    known_domains = ["google.com", "facebook.com", "amazon.com", "apple.com", "microsoft.com", "paypal.com"] # Example list
        ...
        if url_analysis["is_suspicious"]:
            indicators_found.append(f"Suspicious URL found: {url_str} (Reasons: {'; '.join(url_analysis['reasons'])})")
        urls_analyzed_details.append(url_analysis)


    # Crypto Addresses
    for crypto_name, pattern in CRYPTO_ADDRESS_PATTERNS.items():
        if pattern.search(text_content): # Search original text, not lowercased, as patterns might be case-sensitive
    ...
    for case in test_cases:
        ...
        if results['urls_analyzed']:
            ...
            for url_info in results['urls_analyzed']:
                print(f"  - URL: {url_info['url']}, Suspicious: {url_info['is_suspicious']}, Reasons: {url_info.get('reasons', [])}")

        if "expected_min_score" in case:
            assert results['score'] >= case['expected_min_score'], f"Score {results['score']} was less than expected min {case['expected_min_score']}"
            print(f"Assertion: Score >= {case['expected_min_score']} PASSED")
        if "expected_max_score" in case:
            assert results['score'] <= case['expected_max_score'], f"Score {results['score']} was more than expected max {case['expected_max_score']}"
