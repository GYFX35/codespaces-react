...
def is_url_suspicious(url):
    ...
    for legit_substring in ["facebook", "fb", "instagram", "whatsapp"]:
        if legit_substring in domain:
            ...
            return (
                True,
                f"URL contains name of a legitimate service ('{legit_substring}') but is not an official domain.",
            )

    return (
        False,
        "URL does not match common suspicious patterns and is not on the explicit legitimate list.",
    )
...
def analyze_message_for_phishing(message_text):
    ...
    if not findings["keywords_found"] and not findings["suspicious_urls_found"]:
        findings["summary"] = (
            "No immediate phishing indicators found. However, always exercise caution with links and requests for information."
        )
    else:
        summary_parts = []
        if findings["keywords_found"]:
            summary_parts.append(
                f"Found {len(findings['keywords_found'])} suspicious keyword(s): {', '.join(findings['keywords_found'])}."
            )
        if findings["suspicious_urls_found"]:
            summary_parts.append(
                f"Found {len(findings['suspicious_urls_found'])} suspicious URL(s)."
            )
            for sus_url in findings["suspicious_urls_found"]:
                summary_parts.append(
                    f"  - {sus_url['url']} (Reason: {sus_url['reason']})"
                )
        ...
        if findings["score"] > 0:
            findings[
                "summary"
            ] += f" Overall phishing score: {findings['score']} (higher is more suspicious)."
...
if __name__ == "__main__":
    ...
    original_test_messages = [
        (
            "URGENT: Your Facebook account has unusual activity. Please verify your account now by clicking http://facebook.security-update.com/login to avoid suspension.",
            "Original 1",
        ),
        ("Hey, check out this cool site: www.google.com", "Original 2"),
        (
            "Your package is waiting for delivery. Update your shipping details here: http://bit.ly/fakepackage",
            "Original 3",
        ),
        (
            "Hi, this is your bank. We need you to confirm your identity due to a login required. Please visit https://mybank.secure-access-point.net/confirm",
            "Original 4",
        ),
        (
            "A login to your account from a new device was detected. If this wasn't you, please secure your account at http://123.45.67.89/facebook_login",
            "Original 5",
        ),
        (
            "Click here to claim your prize! http://winner.com/prize-claim-form-xyz",
            "Original 6",
        ),
        (
            "Official communication from Facebook: Please review our new terms at https://facebook.com/terms. This is important for your account security.",
            "Original 7",
        ),
    ]
    ...
    additional_test_messages = [
        (
            "Security Alert! Update your info at http://facebook.com.hacker.com and also check this http://bit.ly/anotherlink",
            "Additional 1: Multiple suspicious URLs",
        ),
        (
            "URGENT: verify your account at https://facebook.com/security/alerts - this is a real link, but also check http://mysecurity-fb-check.com",
            "Additional 2: Mix of legit FB URL and suspicious one with keywords",
        ),
        ("Hello there, how are you doing today?", "Additional 3: No keywords, no URLs"),
        (
            "Important security update from Facebook. Please login at https://www.facebook.com to review your settings. Your account safety is our priority.",
            "Additional 4: Keywords but legit URL",
        ),
        (
            "Check this out: http://bit.ly/legitGoogleDoc - this could be a legit shortened link (hard to tell without unshortening)",
            "Additional 5: URL shortener, potentially legit content",
        ),
    ]
