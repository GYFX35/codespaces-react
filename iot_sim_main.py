...
import uuid
from iot_simulator.generators import (
...
def parse_device_profiles(profiles_str_list):
    ...
    if len(profiles_str_list) == 1 and (
        profiles_str_list[0].startswith("[") or profiles_str_list[0].startswith("{")
    ):
        try:
            ...
            if isinstance(parsed_json, list):
                ...
                for p_item in parsed_json:
                    if (
                        ...
                        and "id" in p_item
                        and "sensors" in p_item
                    ):
                        profiles.append(
                            {"id": p_item["id"], "sensors": p_item["sensors"]}
                        )
                    else:
                        print(
                            f"Warning: Invalid JSON profile item format: {p_item}. Skipping."
                        )
                return profiles
            elif isinstance(parsed_json, dict):  # Single profile as JSON object
                if "id" in parsed_json and "sensors" in parsed_json:
                    profiles.append(
                        {"id": parsed_json["id"], "sensors": parsed_json["sensors"]}
                    )
                    return profiles
                else:
                    print(
                        f"Warning: Invalid JSON profile object format: {parsed_json}. Skipping."
                    )
                    return []  # Or handle as error
        except json.JSONDecodeError as e:
            print(
                f"Warning: Could not parse profile string as JSON '{profiles_str_list[0]}': {e}. Proceeding with string parsing."
            )
    ...
    for profile_str in profiles_str_list:
        ...
        if not sensor_types:
            print(
                f"Warning: No sensor types specified for device ID '{device_id}' in profile '{profile_str}'. Skipping."
            )
            continue
...
def main_loop(profiles, interval_seconds, num_messages, output_target, http_url=None):
    ...
    print(
        f"Messages per run (per profile): {num_messages if num_messages > 0 else 'Infinite'}"
    )
        ...
        while True:
            ...
            for profile in profiles:
                ...
                if not all_sensor_readings_for_device:
                    print(
                        f"Warning: No sensor data generated for device {device_id} at {current_ts}. Skipping."
                    )
                    continue

                # Format the payload with all readings for this device
                payload = format_payload(
                    device_id, current_ts, all_sensor_readings_for_device
                )
                ...
                if output_target == "console":
                    ...
                    if http_url:
                        ...
                        print(
                            "Error: HTTP output specified but no URL provided. Skipping send."
                        )

            msg_count += 1
            if (
                num_messages == 0 or msg_count < num_messages
            ):  # Only sleep if not the last message of a finite run
                print(
                    f"--- Sent message batch #{msg_count}. Waiting {interval_seconds}s... ---"
                )
...
def main():
    ...
    if not parsed_profiles:
        print(
            "Error: No valid device profiles could be parsed. Please check your --profiles argument."
        )
        print(
            'Examples: --profiles "myDevice:temperature,humidity" "another:gps,counter"'
        )
        print(
            '      or: --profiles \'[{"id":"dev1","sensors":["temperature","status"]}, {"id":"dev2","sensors":["gps"]}]\''
        )
        return
