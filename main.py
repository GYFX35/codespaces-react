...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            print("\nAnalyzing message...")
            analysis_result = phishing_detector.analyze_message_for_phishing(message_text)

            print("\n--- Phishing Analysis Results ---")
            print(f"Score: {analysis_result['score']} (Higher is more suspicious)")
            if analysis_result['keywords_found']:
                print(f"Suspicious Keywords Found: {', '.join(analysis_result['keywords_found'])}")
            ...
            if not analysis_result['keywords_found'] and not analysis_result['suspicious_urls_found']:
                print("No specific phishing keywords or suspicious URLs detected by basic checks.")
            ...
            profile_url = input("Enter the full Facebook profile URL you want to analyze (e.g., https://www.facebook.com/username):\n").strip()
            if not profile_url.startswith("http://") and not profile_url.startswith("https://"):
                print("Invalid URL format. Please include http:// or https://. Returning to menu.")
                continue
            if not profile_url:
